...
export default function MasjidApp() {
  ...
  return (
    <div
      className={`bg-[var(--bg-body)] text-gray-800 transition-colors duration-500 min-h-screen flex flex-col font-sans`}
    >
      {/* Top Banner */}
              ...
              <div className="relative min-h-[550px] lg:min-h-[650px] flex items-center text-white overflow-hidden border-b-4 border-gold-500">
                ...
                <div className="relative z-20 max-w-7xl mx-auto px-4 py-20 flex flex-col items-center text-center">
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 justify-center w-full max-w-md">
                    <button
                      onClick={() => handleNav("jadwal-sholat")}
              ...
              <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {galleryImages.slice(0, 4).map((img, idx) => (
                    <div
                      key={idx}
                      onClick={() => setLightbox({ isOpen: true, url: img })}
            ...
            >
              ...
              <div className="max-w-5xl mx-auto px-4 py-16 space-y-12">
                <div className="flex flex-wrap gap-2 justify-center">
                  {["semua", "harian", "sholat-jumat", "hari-besar"].map(
                    (f) => (
                      <button
                        key={f}
                        onClick={() => setFilter(f)}
                      ...
                      <div className="p-6 pt-0 border-t border-gold-50/50 flex justify-between items-center text-xs text-gray-500 mt-4">
                        ...
                        <button
                          onClick={() => {
                            ...
                            handleNav("berita-detail");
                          }}
                          className="text-emerald-900 font-bold hover:text-gold-600 transition flex items-center gap-1"
                        >
          ...
          {activeTab === "berita-detail" && selectedNews && (
            ...
            >
              <div className="bg-emerald-900 text-white py-16 text-center relative overflow-hidden border-b-4 border-gold-500">
                ...
                <div className="relative z-10 max-w-7xl mx-auto px-4">
                  <span
                    className={`text-xs ${selectedNews.color} font-bold px-3 py-1 rounded-full uppercase tracking-wider mb-4 inline-block`}
                  >
                    {selectedNews.tag}
                  </span>
                  ...
                  <p className="text-gold-300 mt-4 font-medium flex items-center justify-center gap-2">
                    <CalendarCheck size={16} /> Dipublikasikan pada{" "}
                    {selectedNews.date}
                  </p>
                </div>
              </div>
              <div className="max-w-4xl mx-auto px-4 py-16">
                <button
                  ...
                  className="mb-8 flex items-center gap-2 text-emerald-900 font-bold hover:text-gold-600 transition"
                >
                  <ArrowRight size={16} className="rotate-180" /> Kembali ke
                  Daftar Berita
                </button>
      ...
      <AnimatePresence>
        {toast.isOpen && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 50 }}
            className="fixed bottom-6 left-6 bg-emerald-950 text-gold-100 border border-gold-400 px-5 py-3 rounded-xl shadow-2xl z-50 flex items-center gap-3"
          >
      ...
      <AnimatePresence>
        {lightbox.isOpen && (
          ...
          >
            <button className="absolute top-6 right-6 text-white text-3xl hover:text-gold-300">
              <X size={32} />
            </button>
            <motion.div
              initial={{ scale: 0.8 }}
      ...
      <AnimatePresence>
        {modal.isOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-emerald-950/70 z-50 flex items-center justify-center p-4 backdrop-blur-sm"
          >
            <motion.div
              initial={{ scale: 0.9, y: 30 }}
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.9, y: 30 }}
              className="bg-white rounded-2xl max-w-md w-full p-6 sm:p-8 border-t-4 border-gold-500 shadow-2xl space-y-4"
            >
              <h4 className="font-serif text-xl font-bold text-emerald-950">
                {modal.title}
              </h4>
              ...
              <div className="flex justify-end pt-2">
                <button
                  onClick={() =>
                    setModal({ isOpen: false, title: "", desc: "" })
                  }
                  className="bg-emerald-900 text-gold-100 hover:bg-gold-500 hover:text-emerald-950 font-bold px-5 py-2 rounded-lg text-sm transition"
                >
                  Tutup
                </button>
              </div>
